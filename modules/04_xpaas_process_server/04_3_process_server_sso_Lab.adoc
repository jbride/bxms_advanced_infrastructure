...
.Requirements
* Completion of the previous _Decision_ and _Process_ server labs.
** In particular, you'll need the _gogs_ and _nexus_ pods provisioned in previous lab in your OCP lab environment.
...
In this section of the lab, we generate and import SSL certificates.
These SSL certificates will be used to support link:https://en.wikipedia.org/wiki/Mutual_authentication[2-way mutual authentication] between the _ProcessServer_ and the Red Hat _SSO Server_.
...
. Sign the CSRs with the CA certificate (similar to what a legitimate Certificate Authority would do):
+
[source,text]
----
...
. Import the signed certs into their respective SSL keystores:
+
[source,text]
----
...
. Import out self-signed CA in a truststore:
+
[source,text]
----
...
. Open a web browser, navigate to the URL retrieved from the following:
+
-----
$ echo -en "\nhttps://"$(oc get route/rhsso -o template --template {{.spec.host}})"/auth/admin\n\n"
-----
.. Log in with a username of `admin` and password of `admin`.
...
. In the `Processserver` realm pane, click *Manage -> Import*.
...
* Expect to obtain an encoded _access token_:
+
[source,text]
----
$ echo $TKN
